...
import { deviceProducts } from "@/lib/products";
import ProductCard from "@/components/ProductCard";

export default function DevicePage() {
  const subcats = useMemo(
    () => Array.from(new Set(deviceProducts.map((p) => p.subcat))),
    []
  );
  ...
  const filtered =
    active === "Semua"
      ? deviceProducts
      : deviceProducts.filter((p) => p.subcat === active);

  return (
    <main className="min-h-screen bg-white px-6 py-10 pb-28">
      <h1 className="text-2xl font-bold text-zinc-800 mb-1">DEVICE</h1>
      <p className="text-gray-400 mb-6">Pilih pod system atau mod</p>
